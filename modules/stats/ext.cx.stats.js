	...
	function jsonToTable( records, status ) {
		...
		for ( i = 0; i <= languages.length; i++ ) {
			for ( j = 0; j <= languages.length; j++ ) {
				table[ i ] = table[ i ] || [];
				if ( i === 0 && j === 0 ) {
					table[ 0 ][ 0 ] = mw.msg( 'cx-stats-table-source-target' );
					continue;
				}
				if ( i === 0 ) {
				...
				record = findLanguagePair( records, languages[ i - 1 ], languages[ j - 1 ], status );
				table[ i ][ j ] = record[ 0 ] && record[ 0 ].count || '';
	...
	function prepareStatsTable( records, status ) {
		...
		for ( i = 0; i < table.length; i++ ) {
			...
			for ( j = 0; j < table.length; j++ ) {
				value = table[ i ][ j ];
				$td = $( '<td>' ).text( value ? mw.language.convertNumber( value ) : '' );
				if ( i > 0 && j > 0 && i < table.length - 1 && j < table.length - 1 && value > 0 ) {
	...
	$( function () {
		getCXStats().then( function ( data ) {
			...
			$container.append(
				...
				prepareStatsTable( data.query.contenttranslationstats, 'published' ),
				$( '<h2>' ).text( mw.msg( 'cx-stats-draft-translations-title' ) ),
				prepareStatsTable( data.query.contenttranslationstats, 'draft' ) );
		} );
	} );
}( jQuery, mediaWiki ) );
