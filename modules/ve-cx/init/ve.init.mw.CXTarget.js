 ...
 * references.
 *
 * @param {ve.dm.Document} doc Document the transaction inserts into
 * @param {ve.dm.Transaction} refTx Transaction generated by newFromDocumentInsertion()
 */
ve.init.mw.CXTarget.static.deduplicateReferences = function ( doc, refTx ) {
	for ( let o = 0; o < refTx.operations.length; o++ ) {
		...
		for ( let i = 0; i < refTx.operations[ o ].insert.length; i++ ) {
			...
			if ( kinNode ) {
				...
				// Drop the stale reference body embedded by the translation request, and the
				// original data-mw, so that publishing serializes this node as a name-only
				// reference. Cite regenerates the body from the shared internal list item
				// when this node must carry it. (T428612)
				if ( element.attributes.mw ) {
					delete element.attributes.mw.body;
				}
				delete element.attributes.originalMw;
			}
		}
	}
};
...
ve.init.mw.CXTarget.prototype.setSectionContent = function ( section, content, source ) {
	...
	this.constructor.static.deduplicateReferences( doc, tx );
