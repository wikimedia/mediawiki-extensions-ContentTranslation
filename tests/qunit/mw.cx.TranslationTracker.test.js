...
( function () {
	...
	QUnit.test( 'getSectionNodeValidationTokens', function ( assert ) {
		...
		for ( var i = 0; i < testSet.length; i++ ) {
			var model = ve.dm.converter.getModelFromDom( ve.createDocumentFromHtml( testSet[ i ].html ) );
			var node = model.getDocumentNode().getChildren()[ 0 ];
			var tokens = mw.cx.TranslationTracker.static.getSectionNodeValidationTokens( node, 'en' );
