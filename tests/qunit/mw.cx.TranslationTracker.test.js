...
( function () {
	...
	QUnit.test( 'getSectionNodeValidationTokens', function ( assert ) {
		...
		for ( let i = 0; i < testSet.length; i++ ) {
			const model = ve.dm.converter.getModelFromDom( ve.createDocumentFromHtml( testSet[ i ].html ) );
			const node = model.getDocumentNode().getChildren()[ 0 ];
			const tokens = mw.cx.TranslationTracker.static.getSectionNodeValidationTokens( node, 'en' );
